    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {loading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <KpiCard icon={Users}        label="Membros da Equipe"   value={summary?.totalMembers ?? "—"}   sub="cadastrados no sistema" />
              ...
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="border-b bg-muted/30">
                    <th className="px-4 py-2 text-left text-xs font-semibold text-muted-foreground">Projeto</th>
                    <th className="px-3 py-2 text-left text-xs font-semibold text-muted-foreground">Fase</th>
                    <th className="px-3 py-2 text-left text-xs font-semibold text-muted-foreground">Material</th>
                    <th className="px-3 py-2 text-left text-xs font-semibold text-muted-foreground whitespace-nowrap">Fim Est. Proj.</th>
                    <th className="px-3 py-2 text-left text-xs font-semibold text-muted-foreground whitespace-nowrap">Final Prod.</th>
                    <th className="px-3 py-2 text-left text-xs font-semibold text-muted-foreground whitespace-nowrap">Início Inst.</th>
                  </tr>
                </thead>
                <tbody>
                  {activeProjects.map((p, idx) => {
                    const endAlert = p.endDate ? (() => {
                      const today = new Date(); today.setHours(0,0,0,0);
                      const d = parseISO(p.endDate!); 
                      const diff = Math.floor((d.getTime() - today.getTime()) / 86_400_000);
                      return diff < 0 ? "overdue" : diff <= 7 ? "soon" : null;
                    })() : null;

                    return (
                      <tr key={p.id} className={cn("border-b last:border-0 hover:bg-muted/40 transition-colors", idx % 2 === 1 && "bg-muted/10")}>
                        <td className="px-4 py-2.5">
                          <Link href={`/projects/${p.id}`}>
                            <span className="font-medium text-foreground hover:text-primary transition-colors truncate block max-w-[200px]">{p.name}</span>
                          </Link>
                        </td>
                        <td className="px-3 py-2.5">
                          <Badge variant="outline" className={cn("text-[10px] whitespace-nowrap", STATUS_COLOR[p.status])}>
                        ...
                        <td className="px-3 py-2.5">
                          {p.materialType ? (
                            <Badge variant="secondary" className="text-[10px]">
                              {p.materialType === "madeira" ? "Madeira" : "Alumínio"}
                            </Badge>
                          ) : (
                            <span className="text-muted-foreground/40 text-xs">—</span>
                          )}
                        </td>
                        <td className={cn("px-3 py-2.5 text-xs tabular-nums whitespace-nowrap",
                          endAlert === "overdue" ? "text-red-600 font-semibold" :
                          endAlert === "soon" ? "text-amber-600 font-semibold" :
                          "text-muted-foreground"
                        )}>
                          <span className="flex items-center gap-1">
                            {endAlert === "overdue" && <AlertCircle className="h-3 w-3" />}
                            {endAlert === "soon" && <Clock className="h-3 w-3" />}
                            {fmtDate(p.endDate) ?? "—"}
                          </span>
                        </td>
