  ...
  return (
    <div data-testid={`kanban-card-project-${project.id}`} className={cn(
      ...
      {project.instalacaoStartDate && (
        <div className={cn(
          "rounded-md px-2.5 py-1.5 flex items-center justify-between gap-2",
          instState === "overdue"     ? "bg-red-100 dark:bg-red-900/30 border border-red-300 dark:border-red-700"   :
          instState === "approaching" ? "bg-amber-100 dark:bg-amber-900/30 border border-amber-300 dark:border-amber-700" :
          "bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800"
        )}>
          <div className="flex items-center gap-1.5">
            {instState === "overdue"
              ? <AlertCircle className={cn("h-3.5 w-3.5 text-red-600 dark:text-red-400")} />
              : instState === "approaching"
                ? <AlertCircle className="h-3.5 w-3.5 text-amber-600 dark:text-amber-400" />
                : <CalendarDays className="h-3.5 w-3.5 text-blue-600 dark:text-blue-400" />}
            <span className={cn(
              "text-[11px] font-semibold uppercase tracking-wide",
              instState === "overdue"     ? "text-red-700 dark:text-red-300"   :
              instState === "approaching" ? "text-amber-700 dark:text-amber-300" :
              "text-blue-700 dark:text-blue-300"
            )}>
              {instState === "overdue" ? "Instal. atrasada" : "Instal. prevista"}
            </span>
          </div>
          <span className={cn(
            "text-sm font-bold tabular-nums",
            instState === "overdue"     ? "text-red-700 dark:text-red-200"   :
            instState === "approaching" ? "text-amber-700 dark:text-amber-200" :
            "text-blue-700 dark:text-blue-200"
          )}>
            {formatDate(project.instalacaoStartDate)}
          </span>
        </div>
      )}
    </div>
